...
internal class IngredientCategoryLocaleConfiguration : IEntityTypeConfiguration<IngredientCategoryLocaleEntity>
{
  public void Configure(EntityTypeBuilder<IngredientCategoryLocaleEntity> builder)
  {
    builder.ToTable(nameof(CooxbooxContext.IngredientCategoryLocales), Schemas.Content);
    builder.HasKey(x => new { x.IngredientCategoryId, x.Language });
    ...
    builder.HasOne(x => x.IngredientCategory).WithMany(x => x.Locales).OnDelete(DeleteBehavior.Cascade);
    builder.HasOne(x => x.Kitchen).WithMany(x => x.IngredientCategoryLocales).OnDelete(DeleteBehavior.Restrict);
  }
}
