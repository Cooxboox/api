...
internal class IngredientLocaleConfiguration : IEntityTypeConfiguration<IngredientLocaleEntity>
{
  public void Configure(EntityTypeBuilder<IngredientLocaleEntity> builder)
  {
    builder.ToTable(nameof(CooxbooxContext.IngredientLocales), Schemas.Content);
    builder.HasKey(x => new { x.IngredientId, x.Language });
    ...
    builder.HasOne(x => x.Ingredient).WithMany(x => x.Locales).OnDelete(DeleteBehavior.Cascade);
    builder.HasOne(x => x.Kitchen).WithMany(x => x.IngredientLocales).OnDelete(DeleteBehavior.Restrict);
  }
}
