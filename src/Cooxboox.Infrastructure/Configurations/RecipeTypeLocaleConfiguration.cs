...
internal class RecipeTypeLocaleConfiguration : IEntityTypeConfiguration<RecipeTypeLocaleEntity>
{
  public void Configure(EntityTypeBuilder<RecipeTypeLocaleEntity> builder)
  {
    builder.ToTable(nameof(CooxbooxContext.RecipeTypeLocales), Schemas.Content);
    builder.HasKey(x => new { x.RecipeTypeId, x.Language });
    ...
    builder.HasOne(x => x.RecipeType).WithMany(x => x.Locales).OnDelete(DeleteBehavior.Cascade);
    builder.HasOne(x => x.Kitchen).WithMany(x => x.RecipeTypeLocales).OnDelete(DeleteBehavior.Restrict);
  }
}
